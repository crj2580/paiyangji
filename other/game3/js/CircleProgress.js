...
function init(options, option) {
  ...
  if (canvas) {
    ...
    option.ss =
      typeof options.startAngle === "undefined"
        ? defaultOption.ss
        : options.startAngle;
    option.type = options.type || defaultOption.type;
    option.tt =
      typeof options.endAngle === "undefined"
        ? defaultOption.tt
        : options.endAngle;
    ...
    option.R = option.r * option.precision;
    option._ff = option.total / option.interval;
    option.ff = (option.tt - option.ss) / option._ff;
    option.clockwise = option.tt - option.ss > 0;
    option.endPointColor = options.endPointColor || defaultOption.endPointColor;
    option.endPointR =
      (options.endPointR || defaultOption.endPointR) * option.precision;
...
function drawArc(start, end) {
  ...
  ctx.beginPath();
  ctx.arc(
    option.w / 2 - Math.cos(end - Math.PI) * option.R,
    option.h / 2 - Math.sin(end - Math.PI) * option.R,
    option.endPointR,
    0,
    360
  );
  ctx.fillStyle = option.endPointColor;
  ctx.fill(); 
  ctx.closePath();
}
